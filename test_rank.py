...
from subgraph_extraction.subgraph_extend import subgraph_extend
...
def prepare_features(subgraph, n_labels, max_n_label, n_feats=None):
    ...
    # # 消零
    # dis_nodes_sub = np.where(n_labels[:, 0] == -1)[0]
    # if len(dis_nodes_sub)>0:
    #     label_feats[dis_nodes_sub, max_n_label[0] + 1 + max_n_label[1]] = 0
...
def subgraph_extraction_labeling(ind, rel, A_list, h=1, enclosing_sub_graph=False, max_nodes_per_hop=None, node_information=None, max_node_label_value=None):
    ...
    if len(pruned_subgraph_nodes) <=2:
        pruned_subgraph_nodes, pruned_labels, subgraph_size = subgraph_extend(ind, rel, A_list, h, enclosing_sub_graph, max_nodes_per_hop)
...
def main(params):
    ...
    with torch.no_grad():
        ...
        for triplets in tqdm(neg_triplets):
            head_scores, head_rank, tail_scores, tail_rank = get_rank(triplets, nodes_lables_dic)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset", "-d", type=str, default="WN18RR_v1_ind",
    ...
    parser.add_argument('--test_times', '-tt', type=int, default=1, 
                        help='测试几次并取平均')
    parser.add_argument("--rs_dropout", "-rsd", type=float, default=0.5,
                        help="Dropout rate in relation sementic")
