...
class SubgraphDataset(Dataset):
    ...
    def _prepare_subgraphs(self, nodes, r_label, n_labels):
        subgraph = dgl.DGLGraph(self.graph.subgraph(nodes)) #根据node ID生成子图
        subgraph.edata['type'] = self.graph.edata['type'][self.graph.subgraph(nodes).parent_eid] #返回父图的id
    ...
    def _prepare_features_new(self, subgraph, n_labels, n_feats=None):
        ...
        #     label_feats[dis_nodes_sub, self.max_n_label[0] + 1 + self.max_n_label[1]] = 0
